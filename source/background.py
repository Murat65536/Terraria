...
class Background:
    def __init__(self, images: tuple[str, ...], offset: int, animation_delay: float) -> None:
    ...
    def move(self, amount: float) -> None:
        self.position = self.position + amount if self.surfaces[self.frame].get_width() - self.position - amount > 0 else 0
...
class BackgroundData:
    def __init__(self, *parallaxes: tuple[Background, ...]) -> None:
        self.parallaxes: tuple[tuple[Background, ...], ...] = parallaxes
        self.selected: int = randint(0, len(self.parallaxes) - 1)
    def __iter__(self) -> Generator[Background, None, None]:
        for parallax in self.parallaxes[self.selected]:
            yield parallax
