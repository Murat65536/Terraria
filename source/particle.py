...
class Particle:
    def __init__(
        ...
        self.life = life + (
            random.random() * life * 0.1 - life * 0.05
        )  # How long it lasts for (randomized slightly)
        self.initLife = self.life
        self.color = color
        self.size = (
            size + random.random() * size * 0.1 - size * 0.05
        )  # How large it will be (randomized slightly)
        ...
        if velocity == 0:
            if angle == 0:
                ...
                angle += (
                    random.random() * spread - spread * 0.5
                )  # Set angle + random spread in set range
            self.velocity = (
    ...
    def update(self):
        ...
        self.velocity = (
            ...
            + self.gravity * commons.GRAVITY * commons.DELTA_TIME,
        )
        self.position = (
            self.position[0]
        ...
        self.size = (
            self.initSize * self.life / self.initLife
        )  # Change size based on life and initial size
    ...
    def draw(self):
        if self.outline:
            ...
            border.fill((0, 0, 0))
            border = pygame.transform.rotate(
                border, self.velocity[0] * self.velocity[1] * 10
            )
        ...
        particle = pygame.transform.rotate(
            particle, self.velocity[0] * self.velocity[1] * 10
        )
