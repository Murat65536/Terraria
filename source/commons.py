...
for item in CONFIG_DATA:
    item = item.split("=")
    config_data.append(item[1][:-1])
WINDOW_WIDTH: int = int(config_data[0].split(",")[0])
WINDOW_HEIGHT: int = int(config_data[0].split(",")[1])
GRAVITY: float = 9.8 * BLOCK_SIZE * 0.666 * float(config_data[1])  # 3 tiles = 1 meter
RUN_FULLSCREEN: bool = bool(int(config_data[2]))
PARTICLES: bool = bool(int(config_data[3]))
PARTICLE_DENSITY: float = float(config_data[4])
MUSIC: bool = bool(int(config_data[5]))
CONFIG_MUSIC_VOLUME: float = float(config_data[6])
SOUND: bool = bool(int(config_data[7]))
CONFIG_SOUND_VOLUME: float = float(config_data[8])
CREATIVE: bool = bool(int(config_data[9]))
BACKGROUND: bool = bool(int(config_data[10]))
PARALLAX_AMOUNT: float = float(config_data[11])
PASSIVE: bool = bool(int(config_data[12]))
MAX_ENEMY_SPAWNS: int = int(config_data[13])
FANCY_TEXT: bool = bool(int(config_data[14]))
HITBOXES: bool = bool(int(config_data[15]))
SPLASHSCREEN: bool = bool(int(config_data[16]))
AUTO_SAVE_FREQUENCY: float = float(config_data[17])
EXPERIMENTAL_LIGHTING: bool = bool(int(config_data[18]))
SMOOTH_CAM: bool = bool(int(config_data[19]))
DRAW_UI: bool = bool(int(config_data[20]))
